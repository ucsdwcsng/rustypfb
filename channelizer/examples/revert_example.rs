...
pub use rustdevice::{DevicePtr, compute_bessel};
...
fn main() {
    ...
    let mut revert_filter: Vec<f32> = (0..nch * ntaps)
        .map(|x| {
            ...
            let darg = (2.0 * y) / (chann_float * chann_proto) - 1.0;
            let carg = kbeta * (1.0 - darg * darg).sqrt();
            (compute_bessel(carg) / compute_bessel(kbeta)) * sinc(1.5*arg)
        })
        .collect();
    ...
    let mut output_cpu = vec![Complex::<f32>::zero(); (nch*nslice / 2) as usize];
    ...
    // // Transfer
    revert_output_buffer.dump(&mut output_cpu);
    ...
    ssca_obj.process(&mut output_cpu, false, &mut reverted_sum_buffer, &mut reverted_max_buffer);
    ssca_obj.process(&mut input_vec_complex, false, &mut original_sum_buffer, &mut original_max_buffer);
